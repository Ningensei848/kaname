...
`;
    ...
    assert.strictEqual(sources[0].feed_url, 'https://www.jpcert.or.jp/rss/jpcert.rdf');
    assert.strictEqual(sources[0].custom_extraction_instruction, 'Extract latest alerts');

    fs.unlinkSync(yamlPath);
  });

  await t.test('should skip invalid sources and transition to degraded operation', () => {
