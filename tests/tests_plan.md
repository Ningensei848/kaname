...
## 2. **TDDサイクルによるテストハーネス・契約検証の具体化 (優先度：高):**
   * Fetch バックオフの期待動作は、`tests/fetch.test.ts` 側の決定論的ハーネスとフィクスチャで検証し、再利用実装が必要になった場合は `src/` ではなく将来の別パッケージへ切り出す。
   * HTML ボイラープレート除去の期待動作は、`tests/parser.test.ts` 側のフィクスチャとスキーマ契約で固定し、正規表現ベースの実行ロジックはテストハーネスまたは将来の別パッケージに閉じ込める。
   * SHA-256 べき等性検知は、`tests/state.test.ts` のハーネスと状態フィクスチャで契約を表現し、永続化バックエンドの具体実装は `src/` ではなく fixture/schema 境界または将来の別パッケージに委譲する。
   * インクリメンタル追記の期待動作は、`tests/markdown-updater.test.ts` の差分フィクスチャと Markdown 契約で検証し、本文変換ロジックはテストハーネスまたは将来の別パッケージに配置する。
   
   `src/` は型定義・契約定義のみを保持する境界として扱い、展開されたテストを実行（`pnpm test`）して、テスト計画・TODO・import 構造が同じアーキテクチャ方針を指すことをアサーション検証する。
