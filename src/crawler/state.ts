...
import { CrawlerState, SourceState } from '../types';
...
export function loadCrawlerState(filePath: string): CrawlerState {
  ...
  try {
    ...
    console.warn(`Failed to parse crawler state file at ${filePath}. Starting with initial state. Error: ${(error as Error).message}`);
  }
